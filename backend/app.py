...
def confusion_matrix_api():
    # Baca file pelabelan.csv
    # df = pd.read_csv('pelabelan.csv')
    
    # # Ambil kolom 'aktual' dan 'sentimen'
    # actual = df['aktual'].tolist()
    # predicted = df['sentimen'].tolist()
    
    df = pd.read_csv('pelabelan.csv')
    actual = df['aktual']
    predicted = df['sentimen']
    
    # Hitung Confusion Matrix
    # Hitung confusion matrix
    # Hitung confusion matrix
    cm = confusion_matrix(actual, predicted, labels=['Positif', 'Negatif'])
    positif_positif = cm[0, 0]
    positif_negatif = cm[0, 1]
    negatif_positif = cm[1, 0]
    negatif_negatif = cm[1, 1]

    # Hitung metrik
    accuracy = accuracy_score(actual, predicted)
    precision = precision_score(actual, predicted, pos_label='Positif')
    recall = recall_score(actual, predicted, pos_label='Positif')
    f1 = f1_score(actual, predicted, pos_label='Positif')

    return jsonify({
        'positif_positif': positif_positif,
        'positif_negatif': positif_negatif,
        'negatif_positif': negatif_positif,
        'negatif_negatif': negatif_negatif,
        'accuracy': accuracy,
        'precision': precision,
        'recall': recall,
        'f1score': f1
...
#     ds = pd.read_csv("pelabelan.csv", header=None)

#     print("Data from hasil_vector_matrix.csv:")
#     print(df.head())  # Menampilkan beberapa baris pertama dari data
    
#     print("Data from pelabelan.csv:")
#     print(ds.head())  # Menampilkan beberapa baris pertama dari data
...
#     print("Feature matrix XNum:")
#     print(XNum[:5])  # Menampilkan beberapa baris pertama dari fitur

#     print("Label vector yNum:")
#     print(yNum[:5])  # Menampilkan beberapa label pertama

#     # Menghitung kesamaan cosine
#     similarities = cosine_similarity(XNum, XNum)
    
#     print("Cosine similarities matrix:")
#     print(similarities[:5, :5])  # Menampilkan bagian kecil dari matriks kesamaan
...
#     print(nearest_neighbors[:5])  # Menampilkan tetangga terdekat untuk beberapa baris pertama

#     # Membuat prediksi berdasarkan tetangga terdekat
...
#     print("Predictions:")
#     print(predictions[:5])  # Menampilkan beberapa prediksi pertama

#     # Menghitung akurasi
#     accuracy = np.mean(predictions == yNum)

#     print("Accuracy:")
#     print(accuracy)  # Menampilkan akurasi
...
def KNN():
    ...
    positive_accuracy = np.mean(predictions[yNum == 1] == yNum[yNum == 1])
    negative_accuracy = np.mean(predictions[yNum == 0] == yNum[yNum == 0])
    ...
    return jsonify(accuracy=accuracy, positive_accuracy=positive_accuracy, negative_accuracy=negative_accuracy, ranking=ranking.tolist())
...
@app.route('/add-tweet', methods=['POST'])
def addTweet():
    data = request.json  # Menggunakan request.json untuk data JSON dari frontend
    
    if 'tweet' in data:
        tweet_text = data['tweet']

        # Baca CSV dan tambahkan tweet baru
        df = pd.read_csv('data_tweet.csv', encoding='latin1')
        current_rows = df.shape[0]
        ...
        return "success"  # Mengembalikan 'success' jika berhasil
    else:
        return "error"  # Mengembalikan 'error' jika data 'tweet' tidak ditemukan
...
#     # Get feature names (words) with highest TF-IDF score
#     feature_names = cvect.get_feature_names_out()
#     top_features = []

#     for row in range(normalized_tfidf_matrix.shape[0]):
#         # Find index with maximum TF-IDF value
#         max_index = normalized_tfidf_matrix[row].argmax()
#         top_feature = feature_names[max_index]
#         top_features.append(top_feature)

#     # Create DataFrame to store TF-IDF results
#     tfidf_df = pd.DataFrame(normalized_tfidf_matrix.toarray(), columns=feature_names)
#     tfidf_df['top_feature'] = top_features
...
#     data1 = pd.read_csv('data_tweet.csv')
#     print("Data from hasil_vector_matrix.csv:")
#     print(data.head())  # Menampilkan beberapa baris pertama untuk debugging

#     print("Data from data_tweet.csv:")
#     print(data1.head())  # Menampilkan beberapa baris pertama untuk debugging

#     # Mengambil indeks baris dengan status = 0
#     index_zero_status = data1.loc[data1['status'] == 0].index.tolist()
#     print("Index with status 0:", index_zero_status)

#     # Kolom terakhir yang berisi nilai TF-IDF tidak digunakan untuk penentuan sentimen
#     # Kita menggunakan kolom 'sentimen' dari data_tweet.csv untuk penentuan sentimen

#     sentiments = data1['kelas'].tolist()
#     # sentiments = data1['sentimen'].tolist()

#     print("Sentiments from data_tweet.csv:", sentiments[:10])  # Menampilkan beberapa sentimen pertama untuk debugging
...
def similarity():
    try:
        ...
        df_vectors = pd.read_csv("pelabelan.csv", header=None, skiprows=1)
        vectors = df_vectors.iloc[:, 1:-2].values
        sentiments = df_vectors.iloc[:, -1].values
...
# Mengambil data input yang akan dibandingkan dengan semua data
    ...
    # Mengambil 3 data dengan nilai cosine similarity tertinggi
        ...
        # Example of iterating through top indices and retrieving data
        for i, idx in enumerate(top_indices_sentiments):
            rank = i + 1
            similarity = similarities_sentiments[idx]
            data_number = int(idx + 1)
